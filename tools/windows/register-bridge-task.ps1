...
# Onshape MCP Windows bridge alive across reboots.
#
# Same recovery model as taobao-mcp (DSH_WSL_BRIDGE.md):
#   * Windows Task Scheduler starts bridge_server.py at user logon.
#   * Task restarts the bridge on failure every minute, up to 999 times.
#   * The Linux MCP client owns reconnect/backoff; once 127.0.0.1:8766 is up
...
Register-ScheduledTask `
    ...
    -Trigger $trigger `
    -Settings $settings `
    -Description "Onshape MCP Windows bridge (127.0.0.1:$Port) — one persistent Chrome/Edge profile, single client" `
    -Force
