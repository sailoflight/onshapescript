...
| G5 | Token / retrieval benchmark (capability card vs docs search vs full docs) | **Offline character benchmark delivered in P4** (`test_capability_retrieval`), **extended 2026-09-19** to a three-route size benchmark with a documented token *estimate* (`dev/tools/context_cost.py`, raw output in `onshape_docs/verification/context-cost-2026-09-19.json`): 209 estimated tokens for the actionable card plan against 1106 for the reference search and 1735 for the full guide page. **Extended 2026-09-21** to the tool-list surface and delivered a compression mode: `ONSHAPE_MCP_TOOL_EXPOSURE=gateway` advertises 25 tools (66,051 chars / 16,533 estimated tokens) against 77 tools (167,441 chars / 41,901) for the default `semantic` view and 111 tools (227,459 chars / 56,922) for `static` — 3.4x and 2.5x, with every registered name still callable (`dev/tests/test_tool_gateway_view.py`, raw output in `onshape_docs/verification/context-cost-surfaces-2026-09-21.json`). The compressed view is declarative, not search-only, and it also compacts mutation ANSWERS (one canonical row list plus counts, `include_row_evidence=true` to restore). It did add exactly one registry row, `mcp_tool_invoke`, after a live measurement showed a real client refuses unadvertised names with `unknown tool` — an advertised forwarder through the same handler, so no authority moved. A true tokenizer measurement is still not claimed; the only in-the-loop data point is the P6 capability run. **Follow-up research (same day):** `LOOKUP_DEPTH_RESEARCH.md` prices the layer/entry trade-off — one lookup round costs the whole prefix (~26.3k estimated tokens) versus 427-1,000 tokens/step per advertised entry, three of four prescribed lookup chains were half-listed, and closing them plus the parameter/tab workflows lifts zero-lookup coverage of the recorded task vocabulary from 43% to 86% for +5.5k tokens/step; the same page prices the bridge's collapse/expand layer (one receipt per connection, break-even at expand step 2) and the slimming of `docs_list` (10,301 → 1,811 estimated tokens by default) | G4 |
