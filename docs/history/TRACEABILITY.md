...
| Eight absorbed compatibility names (`fs_list_modules`, `browser_reconnect`, `browser_reload`, `browser_invoke_discovered`, `browser_drawing_insert_views`, `browser_add_drawing_dimension`, `browser_geometry_status`, `browser_configure_geometry_backend`) | `../architecture/TOOL_SURFACE_AUDIT.md` (verdicts now `Internal-only`, follow-up 3 executed 2026-09-19) | Current: registered and callable by exact name as deprecated wrappers with `deprecated` + `useInstead`; not advertised, and the surviving route is named there |
