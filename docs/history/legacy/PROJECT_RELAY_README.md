# 桥接目录：已退役的 Onshape 项目内 WSL 中继 + Windows 主体

> **历史记录，不是运行手册。** 项目自带 relay/listener/launcher 已从源码删除，
> 下文命令和路径仅用于解释迁移前行为，不受支持且不得重新部署。当前权威是
> `../../operations/MCP_RUNBOOK.md`；归档索引见 `../TRACEABILITY.md`。

> The later `SHARED_BRIDGE_MIGRATION.md` captured the extraction target and
> acceptance boundary. That migration is complete; this page preserves the
> former production wiring only as provenance.
...
2. 改 MCP 主体（`mcp_main/win/mcp/server.py`、`onshape_browser_mode/*`、`bridge_server.py`）：
   在 WSL 跑离线测试后，同步到 Windows 部署副本并重启 Windows 桥接。当前
   `C:\MCP\onshapescript` 仍使用 flat compatibility imports（`mcp_main.server` /
   `mcp_main.browser_tools`）；同步 canonical `mcp_main/win/mcp/*.py` 时必须同时更新对应
   flat 文件，直到 Windows launcher 迁移到 `mcp_main.win.mcp`。2026-08-26 的 STEP field
   validation 曾因只同步新分层路径而在 Project executor 找不到新 handler。
