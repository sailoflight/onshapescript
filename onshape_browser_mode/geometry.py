...
from onshape_browser_mode.step_export import OUTPUT_ROOT as STEP_OUTPUT_ROOT
...
def _export_id(value: str) -> str:
    if not isinstance(value, str) or not _ID.fullmatch(value):
        raise ValueError("export_id must be a nonempty opaque identifier")
    return value


def browser_geometry_status(
    config_path: Path = CONFIG_PATH,
    *,
    repo_root: Path = REPO_ROOT,
) -> dict[str, Any]:
    status = command_geometry_status(load_command_geometry_config(config_path))
    if status["ready"]:
...
def configure_browser_geometry_backend(
    candidate_id: str,
    *,
    dry_run: bool = False,
    config_path: Path = CONFIG_PATH,
...
def _load_staged_step(export_id: str, step_output_root: Path) -> StepArtifact:
    staging = step_output_root.resolve() / export_id
    manifest_path = staging / "step-manifest.json"
    if not manifest_path.is_file():
        raise ValueError("browser STEP staging manifest is missing")
    payload = json.loads(manifest_path.read_text(encoding="utf-8"))
    if payload.get("exportId") != export_id:
        raise ValueError("browser STEP staging manifest exportId mismatch")
    artifact = payload.get("artifact")
    if not isinstance(artifact, dict):
        raise ValueError("browser STEP staging manifest artifact is missing")
    relative = PurePosixPath(str(artifact.get("path", "")))
    if relative.is_absolute() or ".." in relative.parts or not relative.parts:
        raise ValueError("browser STEP staging manifest artifact path is invalid")
    source = artifact.get("source")
    if not isinstance(source, dict) or source.get("mode") != "browser":
        raise ValueError("browser STEP staging provenance is not browser-owned")
    path = staging.joinpath(*relative.parts)
    if not path.is_file() or file_sha256(path) != artifact.get("sha256"):
        raise ValueError("browser STEP staging artifact hash verification failed")
    if path.stat().st_size != artifact.get("byteCount"):
        raise ValueError("browser STEP staging artifact byteCount verification failed")
...
def plan_browser_geometry_package(
    export_id: str,
    *,
    config_path: Path = CONFIG_PATH,
    ...
    eid = _export_id(export_id)
    status = browser_geometry_status(config_path)
    source_present = (step_output_root.resolve() / eid / "step-manifest.json").is_file()
    destination = output_root.resolve() / eid
    return {
        "dryRun": True,
        "operation": "browser-step-geometry-package",
        "exportId": eid,
        "semanticLevel": "L6",
...
def build_browser_geometry_package(
    export_id: str,
    *,
    dry_run: bool = False,
    config_path: Path = CONFIG_PATH,
    ...
    if not plan["sourceManifestPresent"]:
        raise RuntimeError("browser STEP staging is unavailable; export or restore it first")
    ...
    if backends is None:
        raise RuntimeError("configured STEP converter backend is unavailable")
    step = _load_staged_step(export_id, step_output_root)
    result = build_geometry_package(
        step,
        output_dir=output_root.resolve() / export_id,
        backends=backends,
