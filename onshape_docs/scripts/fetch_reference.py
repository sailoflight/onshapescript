...
def write_attribution() -> None:
    ...
    lines = [
        ...
        "## Project docs",
        "",
        "The project's own LLM-facing documentation (`guide/*.md`,",
        "`experience/*.md`, selected verification/reference pages, and example docs;",
        "the repository-root README.md stays unindexed) are indexed by",
        ...
        "`onshape_docs/verification/verify_docs.py`. For a question, use the MCP search/list tool over `quick/`,",
        "then retrieve one structured detail from `index/`; never open generated JSON directly",
