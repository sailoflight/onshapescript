...
**上线后第一次实测就抓到了这个假阴性（2026-09-21，部署 89）。** 对同一个 23 行元素
（`ready: true`、`headerCount: 27`）的普通读返回 `rowsComplete: false`：窗口里只有 **22**
个用户行，缺的正是 `TV #gf_pitch = 42 mm …`。修复前这一次读看起来就是「那一行不见了」；
修复后它自己说清楚了「这是一次窗口读」。所以**要行身份就用会滚动的行工具**
（`browser_delete_feature` 的定位器、`enumerate_user_feature_rows` 的滚动并集），
普通读只适合「表头计数 + 已渲染窗口」这类问题。
