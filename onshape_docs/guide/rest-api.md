...
Do not begin with the OpenAPI or HTML under `reference/raw/`. Start with
`onshape_api_list_tags` or `onshape_api_search`, open one exact endpoint/schema/
auth/error entry, and inspect raw provenance only if structured detail is
insufficient. Generated JSON indexes are served by the tools and should not be
read directly.
...
- Every response is ledgered in `onshape_rest_api_mode/config/api-usage.json` (gitignored): 2xx/3xx
...
- Configure the budget in `onshape_rest_api_mode/config/onshape-state.json`:
