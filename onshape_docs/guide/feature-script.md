...
This order is mandatory: candidate index first, one structured detail second,
complete standard-library or raw source only when that detail is insufficient.
Do not open generated JSON index files directly; use the `fs_*` readers.
...
- `onshape_docs/index.json` — categorized guide, experience, verification,
  reference, and example documentation; the repository-root README.md stays a
  human landing page while `onshape_docs/README.md` is the indexed lookup map.
  It uses the same typed-block schema as `guide.json` and is served by `docs_*`. Built
