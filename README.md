...
```
mcp_server.py            compatibility entry point -> mcp_main/server.py
mcp_main/               MCP layer: stdio JSON-RPC server + tool registry/handlers
onshape_docs/           docs + vendored reference + index/query + build scripts
onshape_rest_api_mode/  REST client, Onshape operations, quota guards
...
`onshape_docs/guide/*.md` + `onshape_docs/reference/quick-reference.md` + example docs into `onshape_docs/index.json`;
README.md is the human landing page and is not indexed; the authored `.md` files
stay the originals):

| Tool | Purpose |
|---|---|
| `docs_list` | List every indexed project-doc page (README, onshape_docs/guide/*, quick-reference, example docs) with its section outline |
...
See `onshape_docs/guide/fs-assistant.md` for usage guidance and the tool workflows.
