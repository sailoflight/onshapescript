...
powershell -NoProfile -ExecutionPolicy Bypass -File "mcp_main\win\bridge\windows\register-bridge-task.ps1"
if errorlevel 1 (
...
wscript.exe //B //Nologo "mcp_main\win\bridge\windows\start-bridge-hidden.vbs" 8766
echo Done. The bridge listens on 127.0.0.1:8766 and auto-starts at next logon.
echo Check logs in mcp_main\win\bridge\logs\bridge-server.log
endlocal
