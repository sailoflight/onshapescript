...
TOOLS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "description": (
            ...
            "unreplaced {{PLACEHOLDER}}s, an op* call whose third argument cannot be a definition map, "
            "dimensioned arithmetic mixed with a plain number, and a map literal entry written without "
            "`key : value`) plus warnings for symbols absent from the "
    ...
    },
    {
        ...
        "description": (
            ...
            "browser_* calls. The sign-in page reloads ITSELF while the human types (measured: it replaces "
            "its own document, so typed input and all window-level state are lost), which this server "
            "neither causes nor can prevent or recover; the result therefore carries "
            "`pageMaySelfReload: true` with a `humanInputAdvisory`. Do NOT poll or read that page during "
            "human input, and decide the outcome afterwards with action='health' instead of page reads. "
            "action='release' closes only the browser/context owned by the current MCP "
            ...
            "recovers when its reconnect link is clicked. In resident mode (browser.resident = true) the "
            "browser deliberately outlives this process, so when a call holds no page the probe also reads "
            "that detached browser's own loopback DevTools endpoint, one bounded HTTP read, and reports what "
            "it lists as residentBrowser: an alive, still-logged-in browser is then reported as ok -- with the "
            "endpoint-only evidence stated in the note -- instead of browser_not_running, which now means no "
            "browser answered at all. That read launches nothing, attaches nothing, evaluates no page, and "
            "navigates nowhere. "
